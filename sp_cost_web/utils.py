...
def compute_costs(scenario2erpt=None, 
    ...
    self_deal_net_profit = 0
    for k,v in filp_selfdeal.items():
        if k == 'SP Type':
            continue
        elif k in ['block_rewards', 'income']:
    ...
    df = pd.DataFrame([filp_miner, filp_selfdeal, filp_offset, rd_miner, filp_selfdeal_offset, cc_miner])
